...
class Sequential:
    def __init__(
        ...
        self.optimizer_kwargs = (
            optimizer_hyperparams if optimizer_hyperparams is not None else {}
        )

    def fit(
        self, x: np.ndarray, y: np.ndarray, epochs: int = 1, verbose: bool = False
    ) -> None:
        """
        ...
        for epoch in range(epochs):
            for j, layer in enumerate(self.layers[::-1]):
                ...
                layer.model = self.optimizer(
                    x, y, layer.model, self.costfunction, **self.optimizer_kwargs
                )
