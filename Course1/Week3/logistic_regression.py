...
def run_logistic_regression(x: NDArray, y: NDArray, learning_rate: float = 0.1,
                            max_iter: int = 1000, w: ArrayLike = (0,), b: float = 0,
                            cost_function: CostFunction = logistic_cost_function) -> Model:
    model = LogisticModel(w=w, b=b, n_features=get_n_features(x))
    print(f"Initial cost: {cost_function(x, y, model)}")
    model = batch_gradient_descent(x, y, model, cost_function, learning_rate, max_iter)
    print(f"Final cost: {cost_function(x, y, model)}")
    return model
