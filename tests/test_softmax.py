...
def test_logistic_regression_with_softmax():
    ...
    regression = run_regression(regression_type='softmax', x=dataset_1['X_tmp'], y=dataset_1['y_tmp'].astype(int),
                                w=dataset_1['w_tmp'], b=dataset_1['b_tmp'], max_iter=10000,
                                regularization_param=dataset_1['lambda_tmp'], learning_rate=0.01)

    regression2 = run_regression(regression_type='logistic', x=dataset_1['X_tmp'], y=dataset_1['y_tmp'].astype(int),
                                 w=dataset_1['w_tmp'][0], b=dataset_1['b_tmp'][0], max_iter=10000,
                                 regularization_param=dataset_1['lambda_tmp'], learning_rate=0.01)
    ...
    assert np.allclose(regression.predict(thresh=True), regression2.predict(thresh=True))
    ...
    assert np.shape(regression.model.evaluate(dataset_1['X_tmp'])
                    ) == (dataset_1['y_tmp'].shape[0], dataset_1['w_tmp'].shape[-1])


def test_softmax_3d():
    from sklearn.datasets import make_blobs
    X_train, y_train = make_blobs(n_samples=1000, n_features=2, centers=3, cluster_std=1., random_state=1)

    regression = run_regression(regression_type='softmax', x=X_train, y=y_train.astype(int),
                                w=np.ones([3, 2]), b=np.ones(3), max_iter=20000,
                                regularization_param=0.1, learning_rate=0.01)
